...
// obj to cache tuple contructor function after first run, currently not able to pre-compile
...
let methodMatch (ctx:HttpContext,method:RouteMethod) =
    match method, ctx.Request.Method with
    | RouteMethod.Forward , _        -> true 
    | RouteMethod.Get     , "GET"    -> true
    | RouteMethod.Post    , "POST"   -> true
    | RouteMethod.Put     , "PUT"    -> true
    | RouteMethod.Delete  , "DELETE" -> true
    | RouteMethod.Patch   , "Patch"  -> true
    | _ , _                          -> false

type RouteNode(token:string) =
    let mutable midFns = []
    let mutable endFns = []
    ...
    let mutable edges = Dictionary<char,RouteNode>()
    member __.Edges
        with get() = edges
        and set v = edges <- v
    ...
    static member AddFn (node:RouteNode) fn =
        match fn with
        | Empty -> ()
    ...
    static member Split (node:RouteNode) (pos:int) =
        ...
        let snode = RouteNode(childToken)
    ...
    static member ExtendPath (node:RouteNode) (path:string) (rc:ContType) =
        if path = "" then
            RouteNode.AddFn node rc
            node
        else
            match node.TryGetValue path.[0] with
            | true, cnode ->
                RouteNode.AddPath cnode path rc // recursive path scan
            | false, _    ->
                let nnode = RouteNode(path)
                node.Edges.Add(path.[0], nnode)
                RouteNode.AddFn nnode rc
                nnode

    static member AddPath (node:RouteNode) (path:string) (rc:ContType) =
        ...
        | FullMatch ->
            ...
            node
        | PathInToken ->
            RouteNode.Split node (path.Length)
            RouteNode.AddFn node rc
            node
        | TokenInPath ->
            ...
            | true, cnode ->
                RouteNode.AddPath cnode rem rc // recursive path scan
            | false, _    ->
                let nnode = RouteNode(rem)
                node.Edges.Add(rem.[0], nnode)
                RouteNode.AddFn nnode rc
                nnode
        | SubMatch (i) ->
            RouteNode.Split node (i)
            let rem = path -| i
            let nnode = RouteNode(rem)
            node.Edges.Add(rem.[0],nnode)
            RouteNode.AddFn nnode rc
...
and MidCont =
| ApplyMatch of (char * (char []) * (RouteNode)) // (parser , nextChar(s) , contNode) list
| ApplyMatchAndComplete of ( RouteMethod * char * int * ParseFnCache) // (lastParser, No# Parsers, Cont Fn)
//| MethodMatch of string * RouteNode
    member x.Precedence
        with get () =
            match x with
            //| MethodMatch _ -> 3
            | ApplyMatch (c,_,_) -> (int c)
            | ApplyMatchAndComplete (_,c,_,_) -> 256 + (int c)

and EndCont =
| HandlerMap of (RouteMethod * HttpHandler)
| MatchComplete of (RouteMethod * int * ParseFnCache ) // ( No# Parsers, Cont Fn)
    member x.Precedence
        with get () =
            match x with
            | HandlerMap _ -> 1
...
let inline (=>) (a:HttpHandler -> RouteNode -> RouteNode) (b:HttpHandler) = a b
...
let private getPostMatchNode fmt (nxt:char) (ils:MidCont list) =
    let rec go (ls:MidCont list) (acc:MidCont list) (no:RouteNode option) =
        match ls with
        | [] ->
            match no with
            | None ->
...
///  * `Node`
let route (path:string,method:RouteMethod,fn:HttpHandler,root:RouteNode) =
    // Simple route that iterates down nodes and if function found, execute as normal
    RouteNode.ExtendPath root path ( HandlerMap(method,fn) |> End )
...
///  * `parent` : `Node` - This parameter is applied by `router`, and is ommitted when building api such that function is partially applied fn
///  * `Node`
let routef (path : PrintfFormat<_,_,_,_,'T>,method:RouteMethod,fn:'T -> HttpHandler,root:RouteNode) =
...
// parsing route that iterates down nodes, parses, and then continues down further notes if needed
    ...
    let rec go i ts pcount (node:RouteNode) =
        ...
        if pl < 0 || pl = last then
            ...
            if pcount = 0 then
                ...
                RouteNode.ExtendPath node (path.Value -| ts) ( MatchComplete(method, pcount , ParseFnCache( fun(o:obj) -> (o :?> 'T) |> fn ) ) |> End)  //todo: boxing & upcasting bad for performance, need to fix
        else
            ...
            // formater with valid key
                ...
                if pl + 1 = last then // if finishes in a parse
                    if node.MidFns |> List.exists (function | ApplyMatchAndComplete(m,c,_,_) -> method.Equals m && fmtChar = c | _ -> false )
                    then sprintf "duplicate paths detected '%s', Trie Build skipping..." path.Value |> failwith
                    else
                        RouteNode.ExtendPath node (path.Value.Substring(ts,pl - ts)) (ApplyMatchAndComplete( method ,fmtChar , pcount + 1 , ParseFnCache(fun (o:obj) -> o :?> 'T |> fn )) |> Mid)
                else //otherwise add mid pattern parse apply
                    //get node this parser will be on
                    let nnode = RouteNode.ExtendPath node (path.Value.Substring(ts,pl - ts)) Empty
...
let private processPath (abort:HttpHandler) (root:RouteNode) : HttpHandler =
    ...
    fun next ctx ->
        ...
        let rec crawl (pos:int , node:RouteNode, mf , ef) =
        ...
        let rec checkCompletionPath (pos:int) (node:RouteNode) = // this funciton is only used by parser paths
            //this function doesn't test array bounds as all callers do so before
            let success(pos,node) = struct (true,pos,node)
            let failure(pos)      = struct (false,pos,Unchecked.defaultof<RouteNode>)
        ...
        let rec getNodeCompletion (cs:char []) pos (node:RouteNode) =
            let success(prend,nxtpos,nxtnode) = struct (true,prend,nxtpos,nxtnode)
            let failure                       = struct (false,0,0,Unchecked.defaultof<RouteNode>)
            ...
            | h :: _ ->
                ...
                | HandlerMap(rm,fn) ->                    
                    if methodMatch(ctx,rm) then
                        ...
                        abort next ctx               
                | MatchComplete (rm,i,fn) ->
                    if methodMatch(ctx,rm) then
                        createResult args i fn
                    else abort next ctx                   

        let rec processMid (fns:MidCont list,pos, args) =

            let applyMatchAndComplete pos acc ( rm,f,i,fn ) tail =
                if methodMatch(ctx,rm) then 
...
let private mapNode (fns:(RouteNode->RouteNode) list) (parent:RouteNode) =
    let rec go ls =
        match ls with
        | [] -> ()
        | h :: t ->
...
///  * `parent` : `Node` - This parameter is applied by `router`, and is ommitted when building api such that function is partially applied fn
///  * `Node`
let subRoute (path:string) (fns:(RouteNode->RouteNode) list) (parent:RouteNode) : RouteNode =
    let child = RouteNode.ExtendPath parent path Empty
    mapNode fns child
    child
...
///**Output Type**
///  * `HttpHandler`
let router (abort:HttpHandler) (fns:(RouteNode->RouteNode) list) : HttpHandler =
    let root = RouteNode("")
...
let routerDbg (dbg:string -> unit)  (abort:HttpHandler) (fns:(RouteNode->RouteNode) list) : HttpHandler =
    let root = RouteNode("")
    // precompile the route functions into node trie
    mapNode fns root
